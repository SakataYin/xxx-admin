    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column prop="courseId" label="所属课程">
        <template slot-scope="scope">
          <!--          {{scope.row.courseId}}-->
          <span v-for="course in courses" v-if="scope.row.courseId === course.id">{{course.name}}</span>
        </template>
      </el-table-column>

      <el-table-column prop="courseId" label="考卷">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="380" align="center">
        <template slot-scope="scope">
<!--          <el-button type="default" plain @click="handlePaper(scope.row.id,scope.row.courseId)">手动组卷 <i class="el-icon-document"></i></el-button>-->
...
<!--        <el-transfer  filter-placeholder="请输入选择类型：1选择，2判断，3问答" filterable :filter-method="filterMethod"-->
<!--                      v-model="form2.handleQuestionIds" :data="paperQuestions" :props="{key:'name',lable:'id'}"></el-transfer>-->
<!--      </div>-->
    ...
    <el-dialog title="考卷" :visible.sync="dialogFormVisible2" width="60%" :close-on-click-modal="false">
      <el-card>
        <div v-for="(item,index) in questions" :key="item.id" style="margin: 10px 0">
          <div style="margin: 10px 0; font-size:20px" ><span>{{index+1}}.</span><b>{{item.name}}</b>
...
export default {
  ...
  created() {
    this.load()
    this.request.get("/kcourse").then(res =>{
  ...
  methods: {
    ...
    // generateHandlePaper(){
    //   this.request.post("/paper/handlePaper/", this.form2).then(res => {
    //     if(res.code === '200'){
