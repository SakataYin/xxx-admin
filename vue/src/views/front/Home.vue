<template>
  <div>
    <div style="margin-bottom: 100px;">
      ...
      <div style="border: 1px solid #ccc; border-radius: 10px; margin: 10px 0" v-for="item in tableData" :key="item.id">
        <div style="color: #666; margin: 10px 0; font-size: 20px">{{ item.name }}</div>
        <div style="color: #666;margin: 10px 0;">
          <span>考试时间：{{ item.time }}</span>
          <span style="margin-left: 20px">监考老师：{{ item.teacher }}</span>
          <span style="margin-left: 20px">考试状态：{{ item.status }}</span>
        </div>
        <div style="margin: 10px 0;">
          <el-button style="margin-left: 20px" type="primary" @click="sign(item.id)" >报名</el-button>
          <el-button style="margin-left: 20px" type="primary" @click="doPaper(item.id)" v-if="item.enable && item.status ==='进行中'">考试</el-button>
        </div>
